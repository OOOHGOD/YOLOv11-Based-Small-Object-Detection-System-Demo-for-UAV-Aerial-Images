...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    '''
    源码修改,添加即插即用模块ppa,RFAConv,GSConv,DWSConv
    '''
    base_modules = frozenset(
        {
            Classify,Conv,ConvTranspose,GhostConv,Bottleneck,GhostBottleneck,SPP,SPPF, C2fPSA,C2PSA,DWConv,Focus,BottleneckCSP,C1,
            C2,C2f,C3k2,RepNCSPELAN4,ELAN1,ADown,AConv,SPPELAN,C2fAttn,C3,C3TR,C3Ghost,torch.nn.ConvTranspose2d,
            DWConvTranspose2d,C3x,RepC3, PSA,SCDown,C2fCIB,A2C2f,PPA,RFAConv,GSConv,DWSConv
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # 注意力机制：需要输入通道数的注意力机制,如果不需要通道数的注意力，不用在此添加
        elif m in {CAA,PPA,SHSA}:
            c2 = ch[f]
            args = [c2, *args]
